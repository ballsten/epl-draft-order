...
def create_table(file):
    ...
    season = pd.read_csv(file, sep=",", usecols=COLS)
    ...
    df['posScore'] = (df['points']+(200+df['goaldiff'])/100).astype(str) + df['team']
    df['position'] = pd.to_numeric(df['posScore'].rank(ascending=False), downcast="integer")
...
pd.concat(seasons).to_csv('data/tables/all.csv', quoting=2, index=False, float_format='%.f')
